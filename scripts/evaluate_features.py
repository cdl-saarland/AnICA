...
"Which abstract features where useful?".
"""
...
def handle_campaign(campaign_dir):
    ...
    feature_counter = Counter({k: 0 for (k, v) in actx.insn_feature_manager.features})

    for ab in all_abs:
        for ai in ab.abs_insns:
            exact_scheme_entry = ai.features.get('exact_scheme', None)
            if exact_scheme_entry is not None:
                if not exact_scheme_entry.is_top():
                    feature_counter['exact_scheme'] += 1
                    # If the block is not uniformly interesting or it is
                    # specific to the scheme, the other features will not even
                    # be tried, which would make the other appear useful
    ...
    res_str += "non-top entries per feature:\n"
    for k, v in feature_counter.most_common():
        res_str += f"  {k}: {v}\n"
